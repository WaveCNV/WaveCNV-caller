...
sub EM {
    ...
    for (my $em_iteration=0; $em_iteration < $self->{_max_em_iterations}; 
                                                             $em_iteration++) {
        if ($em_iteration == 0) {
	    if(!@{$self->{_cluster_means}}){
		$self->seed_the_clusters();
	    }
        ...
        foreach my $cluster_index(0..$self->{_K}-1) {         
            $new_means[$cluster_index] = 
                       Math::MatrixReal->new($self->{_data_dimensions},1);        
            $new_means[$cluster_index]->zero();
            foreach my $data_id (keys %{$self->{_data}}) {                        
                my $data_record = $self->{_data}->{$data_id};
                my $data_vec = Math::MatrixReal->new($self->{_data_dimensions},1);
                set_col($data_vec,0,$data_record);
                $new_means[$cluster_index] +=  
        ...
        foreach my $cluster_index(0..$self->{_K}-1) {         
            $new_covariances[$cluster_index] = 
                  Math::MatrixReal->new($self->{_data_dimensions},
...
sub return_individual_class_distributions_above_given_threshold {
    ...
    foreach my $cluster_index (0..$self->{_K}-1) {
        ...
        foreach my $data_id (keys %{$self->{_data}}) {
            my $data_vec = Math::MatrixReal->new($self->{_data_dimensions},1);
            set_col($data_vec, 0, $self->{_data}->{$data_id});
            ...
            if ($self->{_data_dimensions} > 1) {
                ...
                my @var_inverse = as_list($covar->inverse());
                my $var_inverse_val = $var_inverse[0];
                my @data_minus_mean = as_list($datavec_minus_mean);
            ...
            my $coefficient = 1.0 / \
                 ( (2 * PI)**$self->{_data_dimensions} * sqrt($covar->det()) );
...
sub clustering_quality_mdl {
    ...
    foreach my $tag (@{$self->{_data_id_tags}}) {
        ...
        foreach my $cluster_index (0..$self->{_K}-1) {
            ...
            my $covar = $self->{_cluster_covariances}->[$cluster_index];
            my $data_vec = Math::MatrixReal->new($self->{_data_dimensions},1);
            set_col($data_vec, 0, $self->{_data}->{$tag});
            my $datavec_minus_mean = $data_vec - $mean_vec;
            my $exponent = undef;
            if ($self->{_data_dimensions} > 1) {
                $exponent = -0.5 * vector_matrix_multiply( 
                ...
                my @var_inverse = as_list($covar_inverses[$cluster_index]);
                my $var_inverse_val = $var_inverse[0];
                my @data_minus_mean = as_list($datavec_minus_mean);
                my $data_minus_mean_val = $data_minus_mean[0];
                $exponent = -0.5 * ($data_minus_mean_val ** 2) * $var_inverse_val;
...
sub clustering_quality_fisher {
    ...
    my $fisher_trace = 0;
    my $S_w = 
        Math::MatrixReal->new($self->{_data_dimensions}, $self->{_data_dimensions});
    $S_w->zero;
    my $S_b = 
        Math::MatrixReal->new($self->{_data_dimensions}, $self->{_data_dimensions});
    $S_b->zero;
    my $global_mean = Math::MatrixReal->new($self->{_data_dimensions},1);    
    $global_mean->zero;
    foreach my $cluster_index(0..$self->{_K}-1) { 
...
sub find_prob_at_each_datapoint_for_given_mean_and_covar {
    ...
    foreach my $data_id (keys %{$self->{_data}}) {
        ...
        if ($self->{_data_dimensions} > 1) {
            ...
            my @data_minus_mean = as_list($datavec_minus_mean);
            my $data_minus_mean_val = $data_minus_mean[0];
            my @covar_as_matrix = as_list($covar_ref);
        ...
        if ($self->{_data_dimensions} > 1) {
            $coefficient = 1.0 / sqrt( ((2 * PI) ** $self->{_data_dimensions}) * 
                                                                                $covar_ref->det()) ;
        } elsif (!defined blessed($covar_ref)) {
            $coefficient =  1.0 / sqrt(2 * $covar_ref * PI);
        } else {         
            my @covar_as_matrix = as_list($covar_ref);
            my $covar_val = $covar_as_matrix[0];
            $coefficient =  1.0 / sqrt(2 * $covar_val * PI);
        }
        my $prob = $coefficient * exp($exponent);
...
sub K {
	...
	$self->{_cluster_covariances}            = [];
	$self->{_class_priors}                   = [];
	$self->{_class_labels_for_data}          = {};
	$self->{_class_probs_at_each_data_point} = {};
	$self->{_expected_class_probs}           = {};
	$self->{_old_priors}                     = [];
	$self->{_old_old_priors}                 = [];
	$self->{_fisher_quality_vs_iteration}    = [];
	$self->{_mdl_quality_vs_iterations}      = [];
    }
...
sub classify_all_data_tuples_bayes {
    ...
    foreach my $data_id (@{$self->{_data_id_tags}}) {
        my $data_vec = Math::MatrixReal->new($self->{_data_dimensions},1);        
        set_col($data_vec, 0, deep_copy_array($self->{_data}->{$data_id}));
...
sub classify_a_data_point_bayes {
    ...
    foreach my $cluster_index (0..@cluster_mean_vecs-1) {
        ...
        if ($self->{_data_dimensions} > 1) {
            ...
            my @data_minus_mean = as_list($datavec_minus_mean);
            my $data_minus_mean_val = $data_minus_mean[0];
            my @covar_as_matrix = as_list($covariance);
            my $covar_val = $covar_as_matrix[0];
...
sub visualize_clusters {
    ...
    if ($visualization_data_field_width == 2) {
        foreach my $cluster_index (0..$self->{_K}-1) {
            ...
            my $mean = $self->{_cluster_means}->[$cluster_index];
            my $covariance = $self->{_cluster_covariances}->[$cluster_index];
            my ($mux,$muy) = as_list($mean);
            my ($varx,$sigmaxy) = as_list($covariance->row(0+1));
            my ($sigmayx,$vary) = as_list($covariance->row(1+1));
...
sub outer_product {
    my $vec1 = shift;
    my $vec2 = shift;
    my ($nrows1, $ncols1) = $vec1->dim();
    my ($nrows2, $ncols2) = $vec2->dim();
    die "Outer product operation called with non-matching vectors"
        unless $ncols1 == 1 && $ncols2 == 1 && $nrows1 == $nrows2;
    my @vec_arr1 = as_list($vec1);
    my @vec_arr2 = as_list($vec2);
    my $outer_product = Math::MatrixReal->new($nrows1, $nrows2);
    foreach my $index (0..$nrows1-1) {
        my @new_row = map $vec_arr1[$index] * $_, @vec_arr2;
        set_row($outer_product, $index, \@new_row);
    }
...
sub transpose {
    ...
    my $transpose = Math::MatrixReal->new($num_cols, $num_rows);
    foreach my $i (0..$num_rows-1) {
        my @row = as_list($matrix->row($i+1));
        set_col($transpose, $i, \@row );
    }
    return $transpose;
}
...
sub matrix_multiply {
    my $matrix1 = shift;
    my $matrix2 = shift;
    my ($nrows1, $ncols1) = $matrix1->dim();
    my ($nrows2, $ncols2) = $matrix2->dim();
...
#        unless $ncols1 == $nrows2;
    ...
    if ($nrows1 == 1) {
        my @row = as_list($matrix1->row(0+1));
        my @col = as_list($matrix2->column(0+1));
        my $result;
    ...
    my $product = Math::MatrixReal->new($nrows1, $nrows1);
    foreach my $i (0..$nrows1-1) {
        my $row = $matrix1->row($i+1);
        my @product_row;
        foreach my $j (0..$ncols2-1) {
            my $col = $matrix2->column($j+1);
            my $row_times_col = matrix_multiply($row, $col);
            push @product_row, $row_times_col;
        }
        set_row($product, $i, \@product_row);
    }
    return $product;
}
...
sub vector_matrix_multiply {
    ...
    my $product = Math::MatrixReal->new(1, $ncols2);
    my $row = $matrix1->row(0+1);
    my @product_row;
    foreach my $j (0..$ncols2-1) {
        my $col = $matrix2->column($j+1);
...
sub matrix_vector_multiply {
    ...
    my $product = Math::MatrixReal->new($nrows1, 1);
    my $col = $matrix2->column(0+1);
    my @product_col;
    foreach my $i (0..$nrows1-1) {
        my $row = $matrix1->row($i+1);
...
sub set_col {
	...
	$matrix->assign($row+1, $col+1, $data->[$row]);
    }
}
...
sub set_row {
	...
	$matrix->assign($row+1, $col+1, $data->[$col]);
    }
}
