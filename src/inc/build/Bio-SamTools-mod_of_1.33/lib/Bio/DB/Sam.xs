     ...
     if(cgp->rghash) {
        for (i=0;i<n;i++) {
	      ...
	      continue;
           ...
           uint8_t *s = bam_aux_get(pl[i].b, "RG");
           if (!s)
	      continue;

	   khint_t k = kh_get(rg, cgp->rghash, (char*)(s + 1));
           if(k == kh_end(cgp->rghash))
	      continue;
...
AV*
bami_coverage(bai,bfp,ref,start,end,bins=0,filter=newAV(),maxcnt=8000)
    Bio::DB::Bam::Index bai
    Bio::DB::Bam        bfp
    int             ref
    int             start
...
CODE:
  {
	 ...
	 Newxz(cg.bin,bins+1,double);
      ...
      if(flen){
         cg.rghash = kh_init(rg);
         for(i = 0; i < flen; i++) {
     	    elem = av_fetch(filter, i, 0);
	    rgs = SvPV_nolen(*elem);
	    if(rgs) kh_put(rg, cg.rghash, strdup(rgs), &ret);
         }
      }
      ...
      if (cg.rghash) {
         for (k = 0; k < kh_end(cg.rghash); ++k)
            if (kh_exist(cg.rghash, k)) free((char*)kh_key(cg.rghash, k));
         kh_destroy(rg, cg.rghash);
      }
